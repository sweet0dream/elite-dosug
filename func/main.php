<?php
	...
	function format_date($date) {
		return (date('d')==explode('-', explode(' ', $date)[0])[2]?'сегодня':(date('d',time()-86400)==explode('-', explode(' ', $date)[0])[2]?'вчера':explode('-', explode(' ', $date)[0])[2].' '.['01'=>'января','02'=>'февраля','03'=>'марта','04'=>'апреля','05'=>'мая','06'=>'июня','07'=>'июля','08'=>'августа','09'=>'сентября','10'=>'октября','11'=>'ноября','12'=>'декабря'][explode('-', explode(' ', $date)[0])[1]])).(date('Y')!=explode('-', explode(' ', $date)[0])[0]?' '.explode('-', explode(' ', $date)[0])[0].' года':'').' в '.explode(':', explode(' ', $date)[1])[0].':'.explode(':', explode(' ', $date)[1])[1];
	}
